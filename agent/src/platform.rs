...
/// `Global\` because `Local\` is per logon session: an agent started on the
/// desktop and one started over SSH by the same user would each think they
/// were alone (seen 2026-09-24; the pid check caught that one).
pub fn instance_name(base: &Path, role: &str) -> String {
    format!("Global\\QCBridgeAgent.{role}.{}", dir_tag(base))
}
...
mod tests {
    ...
    fn instance_names_differ_by_role_and_directory() {
        ...
        assert_ne!(a, b);
        assert_ne!(a, c);
        assert!(a.starts_with("Global\\QCBridgeAgent.replica."));
